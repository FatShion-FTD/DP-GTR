...
def run_DPParaphrase():
    ...
    for idx, eps in enumerate(paraphrase_epss):
        ...
        for tempreture in temperture_list:
            paraphrased_df[f"T_{tempreture}"] = paraphrased_df['original_question'].apply(
                lambda x: dp_para.privatize(text=x, epsilon=eps)
            )
    ...
    for idx, eps in enumerate(paraphrase_epss):
        ...
        def rewrite(x):
            if DATASET_TYPE == 'medQA':
                rewritten, _, _ = dpmlm.dpmlm_rewrite(x, eps, REPLACE=True, FILTER=True, STOP=True, TEMP=True, POS=True, CONCAT=False)
            else:
                rewritten, _, _ = dpmlm.dpmlm_rewrite(x, eps, REPLACE=True, FILTER=True, STOP=True, TEMP=True, POS=True, CONCAT=True)
...
if __name__ == '__main__':
    # 只针对 DPMLM 与 DPParaphrase 两种实验类型，不包含 HAS
    experiments = [
        # ("csQA", "DPMLM"),
        # ("csQA", "DPParaphrase"),
        ("medQA", "DPMLM"),
    ...
    for ds, exp in experiments:
        ...
        if not os.path.exists(path_prefix):
            os.makedirs(path_prefix)
        print(f"\n============================\nRunning experiment: {EXP_TYPE} on {DATASET_TYPE}\n============================")
        if EXP_TYPE == 'DPMLM':
            run_DPMLM()
        elif EXP_TYPE == 'DPParaphrase':
            run_DPParaphrase()
